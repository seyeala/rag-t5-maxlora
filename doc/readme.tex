...
For Colab-centric workflows where you want to run longer schedules, enable validation-driven early stopping, and capture the training/evaluation loss curves for plotting, you can construct \texttt{TrainConfig} directly and retrieve the loss history that \texttt{train} now returns:
\begin{lstlisting}[language=Python]
from rag_t5.config import load_settings
...
trainer_cfg = TrainConfig(
    ...
    train_path="/content/rag-t5-maxlora/data/processed/alpaca_train.jsonl",
    valid_path="/content/rag-t5-maxlora/data/processed/alpaca_valid.jsonl",
    out_dir="/content/rag-t5-maxlora/outputs/run_001",
...
model, tokenizer, stats = train(trainer_cfg)

import pandas as pd
pd.DataFrame(stats["loss_history"]["eval"]).plot(x="step", y="loss", title="Eval loss")
\end{lstlisting}
This snippet mirrors the training helper used in the provided Colab notebooks and shows how to use the returned ``loss_history`` dictionary for downstream visualization.
